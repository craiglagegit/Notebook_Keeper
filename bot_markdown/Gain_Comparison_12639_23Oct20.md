...
```python
num = 'All'
REPO_DIR = '/project/shared/BOT/rerun/cslage/PTC_LSSTCAM_12639'
...
ptc_data = butler.get('photonTransferCurveDataset', dataId={'detector':94, 'run':run})
mask = ptc_data.expIdMask['C10']
```
...
filename = "/project/cslage/BOT_LSSTCam/eotest/eotest_gain_12644_23oct20.pkl"
...
```python
# Get DM noise results from the doEmpiricalReadNoise=True
filename = "/project/cslage/BOT_LSSTCam/eotest/noise_dm_12639_23oct20.pkl"
file = open(filename, 'rb')
noise = pkl.load(file)
file.close()
print(noise.keys())
...
goodCount = 0
badList = open('/project/cslage/BOT_LSSTCam/eotest/badList_12639.txt', 'w')
goodList = open('/project/cslage/BOT_LSSTCam/eotest/goodList_12639.txt', 'w')
for [EO, name] in [[EO_PTC_gains, 'PTC-Run 12644'], [EO_Fe55_gains, 'Fe55-Run 12643']]:
    ratios = []
    plotCounter += 1
    ...
    plt.ylabel("DM PTC Gains(e-/ADU) (Run 12639); %s Flat Pairs"%num, fontsize = 18)
...
plt.savefig(REPO_DIR+"/plots/PTC_Eotest_Gains_12639_23Oct20.pdf")
badList.close()
goodList.close()
print(goodCount)
```
...
plt.ylabel("EOtest PTC-Run-12644 Gains(e-/ADU); All Flat Pairs", fontsize = 18)
xplot = np.linspace(minGain, maxGain,100)
plt.plot(xplot, xplot, ls = '--', color='blue')
...
plt.text(1.01, n.max(), "Median = %.4f"%np.nanmedian(ratios), fontsize = 18)
plt.text(1.01, 0.9*n.max(), "%d CCDs"%numCCDs, fontsize = 18)
plt.savefig(REPO_DIR+"/plots/Eotest_PTC_Eotest_Fe55_Gains_12644_23Oct20.pdf")
```
...
plt.suptitle("BOT Gains - Run 12639 - 2020-10-23", fontsize=24)
...
plt.savefig(REPO_DIR+'/plots/Gain_Summary_12639_23Oct20.pdf')
...
plt.savefig(REPO_DIR+'/plots/Gain_Histograms_12639_23Oct20.pdf')
```
...
for RAFT in rafts:
    for SENSOR in sensors:
        VENDOR, DETECTOR, raftRow, raftCol = detector(RAFT, SENSOR)
        eotestNoise = ptc_results['total_noise'][RAFT][SENSOR]
        for i, amp in enumerate(amps): 
            noise_DM = noise[str(DETECTOR)][amp]
            if VENDOR == 'ITL':
                EO_noise_ITL.append(eotestNoise[i])
                DM_noise_ITL.append(noise_DM)
            else:
                EO_noise_E2V.append(eotestNoise[i])
                DM_noise_E2V.append(noise_DM)
xplot = np.linspace(0,40,100)

fig = plt.figure(figsize = (8,8))
plt.title("Noise Comparison Run 12639", fontsize = 24)
plt.scatter(EO_noise_ITL, DM_noise_ITL, color = 'green', label = 'ITL')
plt.scatter(EO_noise_E2V, DM_noise_E2V, color = 'red', label = 'E2V')
plt.plot(xplot,xplot,ls = '--', color = 'black')
plt.xlabel("EOTest Total Noise", fontsize = 18)
plt.ylabel("DM Noise from doEmpiricalReaadNoise=True", fontsize = 18)
plt.xlim(0,40)
plt.ylim(0,40)
plt.legend(fontsize=18)
plt.savefig(REPO_DIR+'/plots/Noise_Comparison_12639_23Oct20.pdf')
```
