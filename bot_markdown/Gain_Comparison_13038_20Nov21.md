...
butler = Butler("/repo/main", collections=["LSSTCam/raw/all","LSSTCam/calib",\
                                                    "LSSTCam/calib/u/cslage/20211117A", "u/cslage/bps_13038"])
exposure=3021111500030
```

```python
# Get the eotest results
filename = "/project/cslage/BOT_LSSTCam/eotest/eotest_gain_13038_20nov21.pkl"
...
goodCount = 0
badList = open('/project/cslage/BOT_LSSTCam/eotest/badList_13038.txt', 'w')
goodList = open('/project/cslage/BOT_LSSTCam/eotest/goodList_13038.txt', 'w')
for [EO, name] in [[EO_PTC_gains, 'PTC-Run 13038']]:
    ratios = []
    plotCounter += 1
    ...
    plt.ylabel("DM PTC Gains(e-/ADU) (Run 13038); %s Flat Pairs"%num, fontsize = 18)
...
plt.savefig("/repo/main/u/cslage/bps_13038/plots/PTC_Eotest_Gains_13038_22Nov21.pdf")
badList.close()
goodList.close()
print(goodCount)
```
...
num = 65
plt.figure(figsize=(16,8))
for [EO, name] in [[EO_PTC_a00, 'PTC-Run 13038']]:
    ratios = []
    plotCounter += 1
    plt.subplot(1,2,plotCounter)
    ...
    plt.ylabel("DM PTC A00 (Run 13038); %s Flat Pairs"%num, fontsize = 18)
...
plt.savefig("/repo/main/u/cslage/bps_13038/plots/PTC_Eotest_A00_13038_22Nov21.pdf")
#badList.close()
#goodList.close()
#print(goodCount)
```
...
num = 65
plt.figure(figsize=(16,8))
for [EO, name] in [[EO_PTC_noise, 'PTC-Run 13038']]:
    ratios = []
    plotCounter += 1
    plt.subplot(1,2,plotCounter)
    ...
    plt.ylabel("DM PTC Noise (e-) (Run 13038); %s Flat Pairs"%num, fontsize = 18)
    xplot = np.linspace(minPlot, maxPlot,100)
    plt.plot(xplot, xplot, ls = '--', color='blue')
    slope1 = 1.10
    plt.plot(xplot,slope1*xplot, ls = '--', color='red', label = '%.3f'%slope1)
    ...
    plt.text(1.03, 0.9*n.max(), "%d CCDs"%numCCDs, fontsize = 18)
    plt.text(1.03, 0.85*n.max(), "%d amps"%len(ratios), fontsize = 18)
plt.savefig("/repo/main/u/cslage/bps_13038/plots/PTC_Eotest_Noise_13038_22Nov21.pdf")
#badList.close()
#goodList.close()
#print(goodCount)
```
...
num = 65
badCount = 0
badList = open('/project/cslage/BOT_LSSTCam/eotest/turnoff_clump_13038.txt', 'w')
plt.figure(figsize=(16,8))
for [EO, name] in [[EO_PTC_turnoff, 'PTC-Run 13038']]:
    ratios = []
    plotCounter += 1
    plt.subplot(1,2,plotCounter)
        ...
        for DETECTOR in detectors.keys():
            ...
            for i in range(16):
                ...
                if vendor == VENDOR:
                    ...
                    if name == 'PTC-Run 13038' and eo_turnoff > 75000.0 and \
                    PTC_turnoff[DETECTOR][i] < 60000.0 and VENDOR == 'E2V':
                        badList.write("%s\t%s\t%s\n"%(RAFT,SENSOR,list(ptcDataset.gain.keys())[i]))
                        badCount += 1
    ...
    plt.ylabel("DM PTC Turnoff (ADU) (Run 13038); %s Flat Pairs"%num, fontsize = 18)
    xplot = np.linspace(minPlot, maxPlot,100)
    plt.plot(xplot, xplot, ls = '--', color='blue')
    slope1 = 1.10
    plt.plot(xplot,slope1*xplot, ls = '--', color='red', label = '%.3f'%slope1)
    ...
    plt.text(1.03, 0.9*n.max(), "%d CCDs"%numCCDs, fontsize = 18)
    plt.text(1.03, 0.85*n.max(), "%d amps"%len(ratios), fontsize = 18)
plt.savefig("/repo/main/u/cslage/bps_13038/plots/PTC_Eotest_Turnoff_13038_23Nov21.pdf")
badList.close()
#goodList.close()
print(badCount)
```
