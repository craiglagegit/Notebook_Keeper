...
```python
DATA_DIR = '/project/shared/BOT/rerun/cslage/PTC_LSSTCAM_12639'
RAFT = 'R22'
SENSOR ='S11'
DETECTOR = 94
expId1 = 3020101200550
expId2 = 3020101200554
butler = Butler(DATA_DIR)
```

```python
# Now let's get the postISRCCD files.
exp1 = butler.get('postISRCCD', raftName=RAFT, detectorName=SENSOR, expId=expId1)
exp2 = butler.get('postISRCCD', raftName=RAFT, detectorName=SENSOR, expId=expId2)
```
...
mean = np.mean(arr)
std = np.std(arr)
img1 = ax1.imshow(arr, norm=LogNorm(vmin=.01, vmax=10))
colorbar(img1)
ax1.set_title("PostISRCCD %s %s%s; Mean = %.3f, Std = %.3f"%(RAFT,SENSOR,expId1,mean, std))
arr = exp2.image.array
mean = np.mean(arr)
std = np.std(arr)
img2 = ax2.imshow(arr, norm=LogNorm(vmin=.01, vmax=10))
colorbar(img1)
ax2.set_title("PostISRCCD %s %s%s; Mean = %.3f, Std = %.3f"%(RAFT,SENSOR,expId2,mean, std))
