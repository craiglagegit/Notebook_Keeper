...
file.close()
# Get the "badAmps" results.  These are amps where the gains differed by more than 7%
filename = "/project/cslage/BOT_LSSTCam/eotest/badList_12606.txt"
...
print(len(badAmps),badAmps)
filename = "/project/cslage/BOT_LSSTCam/eotest/goodList_12606.txt"
file = open(filename, 'r')
lines = file.readlines()
file.close()
goodAmps = []
for line in lines:
    items = line.split()
    goodAmps.append(items)
print(len(goodAmps),goodAmps)
```
...
pdf = PdfPages(REPO_DIR+"/plots/Gain_Differences_GT_5Pct_20Oct20.pdf")
...
```python
# Compare badList results between two runs
...
filename = "/project/cslage/BOT_LSSTCam/eotest/badList_12638.txt"
file = open(filename, 'r')
