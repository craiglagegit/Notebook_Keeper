...
goodCount = 0
badList = open('/project/cslage/BOT_LSSTCam/eotest/badList_13144B.txt', 'w')
zeroList = open('/project/cslage/BOT_LSSTCam/eotest/zeroList_13144B.txt', 'w')
goodList = open('/project/cslage/BOT_LSSTCam/eotest/goodList_13144B.txt', 'w')
for [EO, name] in [[EO_PTC_gains, 'PTC-Run 13144B']]:
    ratios = []
    plotCounter += 1
        ...
        for DETECTOR in detectors.keys():
            ...
            for i in range(16):
                ...
                if vendor == VENDOR:
                    ...
                    if name == 'PTC-Run 13144B' and (ratio < 0.001):
                    ...
                    elif name == 'PTC-Run 13144B' and (ratio < 0.95 or ratio > 1.05):
                        # 20% - 5 bad, 10% - 8 bad, 5% - 16 bad
                        badCounts[int(DETECTOR)] += 1
                        badList.write("%s\t%s\t%s\n"%(RAFT,SENSOR,list(ptcDataset.gain.keys())[i]))
                        print(ratio, VENDOR, RAFT, SENSOR, DETECTOR, list(ptcDataset.gain.keys())[i],PTC_gains[DETECTOR][i],EO[DETECTOR][i])
                    elif name == 'PTC-Run 13144B' and (ratio > 0.95 and ratio < 1.05):
...
plt.savefig("/repo/main/u/cslage/bps_13144B/plots/PTC_Eotest_Gains_13144B_16Dec21.pdf")
badList.close()
goodList.close()
print(goodCount)
```
...
plt.savefig("/repo/main/u/cslage/bps_13144B/plots/PTC_Eotest_A00_13144B_16Dec21.pdf")
#badList.close()
#goodList.close()
#print(goodCount)
```
...
for [EO, name] in [[EO_PTC_noise, 'PTC-Run 13144']]:
    ...
    plt.text(1.03, 0.9*n.max(), "%d CCDs"%numCCDs, fontsize = 18)
    plt.text(1.03, 0.85*n.max(), "%d amps"%len(ratios), fontsize = 18)
plt.savefig("/repo/main/u/cslage/bps_13144B/plots/PTC_Eotest_Noise_13144B_16Dec21.pdf")
#badList.close()
#goodList.close()
#print(goodCount)
```
...
num =343
badCount = 0
badList = open('/project/cslage/BOT_LSSTCam/eotest/turnoff_clump_13144B.txt', 'w')
plt.figure(figsize=(16,8))
for [EO, name] in [[EO_PTC_turnoff, 'PTC-Run 13144']]:
    ...
    plt.text(1.03, 0.9*n.max(), "%d CCDs"%numCCDs, fontsize = 18)
    plt.text(1.03, 0.85*n.max(), "%d amps"%len(ratios), fontsize = 18)
plt.savefig("/repo/main/u/cslage/bps_13144B/plots/PTC_Eotest_Turnoff_13144B_16Dec21.pdf")
badList.close()
#goodList.close()
print(badCount)
```
