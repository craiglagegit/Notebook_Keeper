...
butler = Butler("/repo/main", collections=["LSSTCam/raw/all","LSSTCam/calib",\
                                                    "u/cslage/calib/13144/calib.20211229"])
camera = butler.get('camera', instrument='LSSTCam')
```

```python
linPtcButler = Butler("/repo/main", collections=["u/cslage/bps_13144H"])
```

```python
nonlinPtcButler = Butler("/repo/main", collections=["u/cslage/bps_13144C"])
```
...
```python
expId=3021120700200
pdf = PdfPages("/repo/main/u/cslage/bps_13144H/plots/Linearity_Results_13144H_29Dec21.pdf")
...
for i, det in enumerate([55, 74]):
    ...
    for amp in camera[0].getAmplifiers():
        ...
        plt.plot(inputAbscissa[plotMask], (modelOrdinate-linearOrdinate)[plotMask], ls = '--', lw=3, color='red', label="Spline fit")
        plt.scatter(binCenters / linearFit[1], values, marker = 'x', s = 200, color='red', label="Spline knots")
        plt.scatter(inputAbscissa[plotMask], (inputOrdinate - linearOrdinate)[plotMask], label="Input data")
        ...
        plt.subplot(2,4,7)
        plt.title("Spline fit to MonDiode data")
        plt.plot(linearOrdinate[plotMask], (modelOrdinate-linearOrdinate)[plotMask], ls = '--', lw=3, color='red', label="Spline fit")
        plt.scatter(binCenters, values, marker = 'x', s = 200, color='red', label="Spline knots")
        plt.scatter(linearOrdinate[plotMask], (inputOrdinate - linearOrdinate)[plotMask], label="Input data")
        plt.xlabel("Flux (ADU)")
        plt.ylabel("Deviation from Linearity(ADU)")
        plt.xlim(0, 100000)
        plt.xticks([0,25000,50000,75000,100000])
...
```python
(modelOrdinate[fluxMask]-linearOrdinate[lowMask])
```

```python
len(inputOrdinate[lowMask]-linearOrdinate[lowMask])
```

```python
plt.scatter(inputAbscissa[lowMask], (inputOrdinate[lowMask] - linearOrdinate[lowMask]), label="Input data")
```
