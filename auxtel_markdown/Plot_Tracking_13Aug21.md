...
start = Time(mDatas[expIds[0]]['DATE-BEG'],format='isot', scale='utc')
end = Time(mDatas[expIds[1]]['DATE-END'],format='isot', scale='utc')
timestamp = f"time >= '{start}+00:00' AND time <= '{end}+00:00'"
...
ax1.axvline(Time(mDatas[expIds[1]]['DATE-BEG'], scale='utc').isot, color='cyan', linestyle="--", label="Exp2_Start")
ax1.axvline(Time(mDatas[expIds[1]]['DATE-END'], scale='utc').isot, color='magenta', linestyle="--", label="Exp2_End")
