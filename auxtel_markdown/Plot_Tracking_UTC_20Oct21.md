...
```python
# Get and plot the data
# If I shift tstart and tend by 37 seconds, then the plot makes sense.

az = await client.select_packed_time_series("lsst.sal.ATMCS.mount_AzEl_Encoders", "azimuthCalculatedAngle",  
                                            tstart- TimeDelta(tai_delta, format='sec'), tend- TimeDelta(tai_delta, format='sec'), is_window=True)
print(f"Tstart={tstart}, Start of dataFrame = {az.index[0]}")
...
ax1.set_ylabel("Degrees")
ax1.legend()
plt.savefig(f"/project/cslage/AuxTel/offsets/Tracking_Timebase_{expId}_TAI_Shift_20Oct21.pdf")
